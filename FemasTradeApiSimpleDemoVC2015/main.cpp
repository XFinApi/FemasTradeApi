...
2.拷贝XTA_W32\Api\Femas_vX.XX目录（X.XX为版本号）下的所有dll文件到可执行文件目录，
...
{
	printf("  PreBalance=%g, Available=%g, CurrMargin=%g, Commission=%g, PositionProfit=%g, CloseProfit=%g, FrozenMargin=%g, FrozenCommission=%g, Withdraw=%g, Deposit=%g\n",
		DEFAULT_FILTER(acc.PreBalance), DEFAULT_FILTER(acc.Available), DEFAULT_FILTER(acc.CurrMargin), DEFAULT_FILTER(acc.Commission),
		DEFAULT_FILTER(acc.PositionProfit), DEFAULT_FILTER(acc.CloseProfit), DEFAULT_FILTER(acc.FrozenMargin), DEFAULT_FILTER(acc.FrozenCommission),
...
void MarketTest()
{
	...
	int err = -1;
#ifdef _DEBUG
	market = XFinApi_CreateMarketApi("XTA_W32/Api/Femas_v1.03/XFinApi.FemasTradeApid.dll", &err);
#else
	market = XFinApi_CreateMarketApi("XTA_W32/Api/Femas_v1.03/XFinApi.FemasTradeApi.dll", &err);
#endif
...
void TradeTest()
{
	...
	int err = -1;
#ifdef _DEBUG
	trade = XFinApi_CreateTradeApi("XTA_W32/Api/Femas_v1.03/XFinApi.FemasTradeApid.dll", &err);
#else
	trade = XFinApi_CreateTradeApi("XTA_W32/Api/Femas_v1.03/XFinApi.FemasTradeApi.dll", &err);
#endif
	if (err || !trade)
	{
